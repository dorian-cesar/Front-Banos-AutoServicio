...
async function fetchWithToken(
  ...
  // Si no hay token, intentamos obtener uno antes de la primera petición
  if (!token && maxRefreshAttempts > 0) {
    if (!refreshingPromise) {
      refreshingPromise = (async () => {
        try {
          console.log("[banioService] No hay token inicial, autenticando...");
    ...
    try {
      ...
      throw new Error(
        "No se pudo obtener el token inicial: " + (err.message || err),
      );
    }
  }
  ...
  if (res && res.status === 401 && maxRefreshAttempts > 0) {
    ...
    if (refreshingPromise) {
      ...
      refreshingPromise = (async () => {
        try {
          console.log("[banioService] Token expirado (401), reautenticando...");
          const creds = await getCredentials();
          const loginResp = await login(creds.email, creds.password);
          if (!loginResp || !loginResp.token)
            throw new Error("Login no devolvió token");
